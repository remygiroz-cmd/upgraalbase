...
function WorkTaskCard({ task, onComplete, onUncomplete, onRemove, onUpdateQuantity }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "p-3 sm:p-4 rounded-xl border-2 transition-all h-full flex flex-col",
        task.is_completed
        ...
        {taskDetails?.image_url && (
          <div className="w-full">
            <img 
              ...
              alt={task.task_name}
              className={cn(
                "w-full h-32 sm:h-40 rounded-lg object-cover border-2",
        ...
        <div className="flex flex-col gap-2 sm:gap-3 flex-1">
          <div className="flex-1 flex flex-col">
            <div className="flex items-start gap-2 flex-wrap mb-2">
              <h4 className={cn(
                  "font-bold text-gray-900 text-sm sm:text-base break-words",
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {hasQuantity && !task.is_completed && (
                <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-indigo-600/20">
                  <button
                    onClick={() => {
                    ...
                    className="w-8 h-8 rounded-md bg-white/20 hover:bg-white/30 active:bg-white/40 flex items-center justify-center transition-all active:scale-95 touch-manipulation"
                  >
              ...
              {hasQuantity && task.is_completed && (
                <span className="px-2 py-1 rounded-lg bg-indigo-600/20 text-indigo-400 text-xs font-medium break-words">
                  Qté : {task.quantity_to_produce} {taskDetails?.unit || ''}
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 mt-auto pt-2">
            {!task.is_completed && (
              <>
                <Button
                  ...
                  variant="ghost"
                  onClick={onRemove}
                  className="text-gray-600 hover:text-red-600 hover:bg-red-50 min-h-[40px] sm:min-h-[44px] min-w-[40px] sm:min-w-[44px] px-2"
                >
                  <X className="w-4 h-4 sm:w-5 sm:h-5" />
                </Button>
                <Button
                  onClick={onComplete}
                  className="bg-orange-600 hover:bg-orange-700 min-h-[40px] sm:min-h-[48px] flex-1 text-sm sm:text-base font-semibold"
                >
            ...
            {task.is_completed && (
              <Button
                ...
                variant="outline"
                className="border-orange-600 text-orange-600 hover:bg-orange-50 min-h-[40px] sm:min-h-[44px] w-full text-sm sm:text-base font-semibold"
              >
