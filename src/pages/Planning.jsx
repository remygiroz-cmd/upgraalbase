...
export default function Planning() {
  ...
  const { data: _pvs = [] } = useQuery({ queryKey: ['planningViewSettings'], queryFn: () => base44.entities.AppSettings.filter({ setting_key: 'planning_view' }) });
  const planningViewRecord = _pvs[0] || null;
  useEffect(() => { if (planningViewRecord) { setColumnOrder(planningViewRecord.column_order || []); setHiddenColumns(planningViewRecord.hidden_columns || []); } }, [planningViewRecord?.id, planningViewRecord?.updated_date]);
  const saveViewSettingsMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['planningViewSettings'] })
  });
                        ...
                        <div className="bg-gradient-to-r from-gray-200 to-gray-100 border-b-2 border-gray-400 flex">
                          <div className="sticky left-0 z-20 bg-gradient-to-r from-gray-200 to-gray-100 border-r-2 border-gray-400 px-1 lg:px-2 py-2 lg:py-3 shadow-sm w-[80px] lg:w-[120px]">
                            ...
                            {(() => {
                              ...
                              return (
                              <button onClick={() => { if (!canModifyPlanning) { toast.error('Lecture seule', { duration: 3000, icon: '🔒' }); return; } handleCopyWeekFromAbove(weekStart); }}
                                               disabled={!canModifyPlanning}
                                               className={cn("text-[8px] lg:text-[9px] px-1 lg:px-1.5 py-0.5 lg:py-1 text-white rounded flex items-center gap-0.5 lg:gap-1 w-full justify-center font-semibold shadow-sm transition-colors", canModifyPlanning ? "bg-blue-600 hover:bg-blue-700" : "bg-gray-400 cursor-not-allowed opacity-60")}
                              ...
                              return (
                                <div key={employee.id} className="border-r border-gray-200 min-w-[150px] w-[150px] lg:min-w-[180px] lg:w-[180px]">
                                  <WeeklySummary
                                    ...
                                    onRecapUpdate={async () => {
                                                       await queryClient.invalidateQueries({ queryKey: ['allWeeklyRecaps'] });
                                                       await queryClient.refetchQueries({ queryKey: ['allWeeklyRecaps', currentYear, currentMonth], exact: true });
                                                     }}
                                    nonShiftEvents={nonShiftEvents}
                                    nonShiftTypes={nonShiftTypes}
                                    disabled={!canModifyPlanning}
