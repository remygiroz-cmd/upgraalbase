...
import PlanningFAB from '@/components/planning/PlanningFAB';
...
export default function Planning() {
  ...
  const [showClearAllEmployeesMonthModal, setShowClearAllEmployeesMonthModal] = useState(false);
  ...
  return (
    <div className="space-y-2">
      ...
      <PlanningFAB
        ...
        canDoDirectSwap={canDoDirectSwap}
        onLeaveRequest={() => setShowLeaveRequestModal(true)}
        onShiftSwap={() => setShowShiftSwapModal(true)}
        onDirectSwap={() => setShowDirectSwapModal(true)}
        onAddCP={() => setModalState({ isOpen: true, actionType: 'ADD_CP', selectedEmployee: null })}
        onApplyTemplate={() => setModalState({ isOpen: true, actionType: 'APPLY_TEMPLATE', selectedEmployee: null })}
        onResetMonth={() => setShowClearMonthModal(true)}
        onClearAllEmployeesMonth={() => setShowClearAllEmployeesMonthModal(true)}
        onExportCompta={() => setShowExportComptaModal(true)}
      />
      </div>
      );
      }