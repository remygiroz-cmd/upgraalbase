...
export default function ClearAllEmployeesMonthModal({ open, onOpenChange, year, month, monthKey }) {
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      await Promise.all([
        ...
        ...allMonthlyRecaps.map(r => base44.entities.MonthlyRecap.delete(r.id)),
      ]);

      const total = shiftsToDelete.length + nonShiftsToDelete.length + cpToDelete.length + weeklyRecapsToDelete.length + allMonthlyRecaps.length;
      ...
      toast.success(`✓ ${total} élément(s) supprimé(s) pour tous les employés — ${monthName}`);
      onOpenChange(false);
    } catch (err) {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="text-red-600 flex items-center gap-2">
            ...
            Vider le mois — Tous les employés
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 flex gap-2">
            ...
            <div className="text-sm text-red-800">
              ...
              <p>Tous les éléments suivants pour <strong>{monthName}</strong> et <strong>tous les employés</strong> seront supprimés :</p>
              <ul className="mt-1 list-disc list-inside text-xs space-y-0.5">
                <li>Shifts et non-shifts</li>
                <li>Périodes de congés payés</li>
                <li>Récapitulatifs hebdomadaires et mensuels</li>
              </ul>
            </div>
          </div>
