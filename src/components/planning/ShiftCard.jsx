              ...
              <div className="flex items-center gap-0.5">
                ...
                <button
                   ...
                   className={cn(
                     ...
                     shift.clock_clicked ? "hover:bg-red-100" : "hover:bg-green-100"
                   )}
                   title={disabled ? "Lecture seule" : "Mettre fin maintenant"}
                 >
                  {isSettingEndNow ? (
                    <div className={cn("w-3 h-3 border-2 border-t-transparent rounded-full animate-spin", shift.clock_clicked ? "border-red-600" : "border-green-600")} />
                  ) : (
                    <Clock className={cn("w-3 h-3", shift.clock_clicked ? "text-red-600" : "text-green-600")} />
                  )}
                </button>
              </div>
            )}
      ...
      {swapInfo && (
        <div className="mt-1 pt-1 border-t border-purple-200 relative">
          ...
          <div className="text-[8px] text-purple-500 leading-tight mt-0.5">
            <span>Initialement le {swapInfo.originalDate ? new Date(swapInfo.originalDate + 'T00:00:00').toLocaleDateString('fr-FR', { day:'2-digit', month:'2-digit' }) : ''}</span>
          </div>
        </div>
      )}
    </div>
